...
const unsigned long UPDATE_INTERVAL = 100; // Update every 5 seconds
...
void checkBatteryLevel() {
  if (batteryLevel <= 10.0) {
    ...
    stopMotors();
    Serial.println("Bateria Baja. Robot detenido.");
    Firebase.setString(fbdo, F("/robot/estado"), F("Detenido por bateria baja"));
...
String getStatusString() {
  if (isOperating) {
    if (isMapping) return "Mapeando";
    if (isDefiningArea) return "Definiendo Areá";
    return "Operando";
  }
  if (isReturningToBase) return "Regresando a la Base";
  return "Inactivo";
}
